...
function Post({
    ...
    return (
        <div key={item.post.id} className={`post`}>
            <div className='NameBuddy'>
                <div className='profile-public-news'>
                    <div className='image-news-avatar'>
                        ...
                        <img
                            src={item.post.urlAvatar}
                            alt={`${item.post.teamName} Avatar`}
                    ...
                    <div className='name-curator-a-href'>
                        <h3>{item.post.teamName}</h3>
                        <a href='#ff'>Team#{item.post.teamNumber}</a>
                    </div>
                </div>
                <div className='date-container'>
                    {(item.role === 'admin' || (item.role === 'curator' && login === item.post.curator)) && (
                        <span className='delete-icon' onClick={() => handleDeletePost(item.post.id)}>
                    ...
                    <p>{formatDate(item.post.date)}</p>
                </div>
            </div>
            <div className='image-news'>
                {imageLoading.image && <div className='loader'></div>}
                <img
                    src={item.post.urlPostImage}
                    alt={`${item.post.teamName} Post`}
